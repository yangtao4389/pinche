...
(function($, window, document) {
	...
	var IndexedList = $.IndexedList = $.Class.extend({
		...
		caleLayout: function() {
		    var self = this;
		    var withoutSearchHeight = (self.box.offsetHeight - (!self.el.search?0: self.el.search.offsetHeight)) + 'px';
		...
		search: function(keyword) {
			...
			var checkGroup = function(currentIndex, last) {
				if (itemCount >= currentIndex - groupIndex - (last ? 0 : 1)) {
					selectorBuffer.push("#"+self.box.id +" "+classSelector('indexed-list-inner li') + ':nth-child(' + (groupIndex + 1) + ')');
			...
			liArray.forEach(function(item) {
				...
				if (item.classList.contains($.className('indexed-list-group'))) {
					...
					if (keyword && text.indexOf(keyword) < 0 &&
					    ...
					    selectorBuffer.push("#" + self.box.id + " " + classSelector('indexed-list-inner li') + ':nth-child(' + (currentIndex + 1) + ')');
						itemCount++;
					}
